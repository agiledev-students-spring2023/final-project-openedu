...
export async function updateProfile(token, newParams) {
  ...
  const cleanedParams = cloneObject(
    newParams,
    "pwd",
    "userId",
    "mock",
...
export async function updatePost(postId, params) {
  const post = await getModel("posts").findOne({ postId: postId });

  const cleanedParams = cloneObject(params, "userId", "postId", "token");
...
export async function getOneFeed(userId, feedId) {
  return await getModel("feedbacks").findOne({
    userId: userId,
    feedId: feedId,
  });
}
